...
cant_20, cant_10, cant_5, cant_2 = 5, 10, 20, 20

usuarios = {"mauricio": 1234, "jose": 4567, "andres": 8901} # USUARIOS Y CONTRASEÑAS
print("( Usuarios y pin ",usuarios," )") # Mostramos para efectos de pruebas
lista_usuarios = list(usuarios.keys()) # pasamos las claves del diccionario a una lista 
cliente = str
...
while True:  # retornamos a la pantalla de inicio
    ...
    if salu2 > 1: print("\n      Bienvenido al Banco Nacional\n".upper())  
    while True:
        cliente = str(input("Nombre de usuario: > ").lower())
        if cliente in usuarios:
            r = usuarios.get(cliente)
            break
    ...
    for i in range(intentos):  # validamos solo 3 intentos de pin fallido
         if pin != r:
             ...
             x.append(pin)
             if x[i]!= r and i < intentos - 1: print("PIN incorrecto, le quedan ",intentos - i - 1," intentos") # no se muestra el " 0 intentos"   
             if i == intentos -1 and pin != r:
                     print("Su tarjeta sera bloqueada")
             if x[i] == r:
                 if cliente == lista_usuarios[0]:
                     ...
                     saldo = 100000
                 elif cliente == lista_usuarios[1]:
                     transferencia = {"PEDRO": 0, "DAVID": 0, "ALEJANDRO": 0, "ANGIE": 0, "CAROL": 0} # opciones de nombres a quien se le pueden hacer transferecias
                     pago_servicios = {"AGUA": 7625, "ELECTRICIDAD": 25900, "TELEFONO": 18700} # opciones de pago de servicios
                     saldo = 150000
                 elif cliente == lista_usuarios [2]:
                     transferencia = {"JIMMY": 0, "VANESA": 0, "REBECA": 0, "LUIS": 0, "FERNANDO": 0} # opciones de nombres a quien se le pueden hacer transferecias
                     pago_servicios = {"AGUA": 12355, "ELECTRICIDAD": 41500, "TELEFONO": 54000} # opciones de pago de servicios
                     saldo = 200000
                     
# cuando ingresamos solicitamos la respuesta de S o N para otra transaccion        
                 while SoN != "n" and SoN != "s": SoN = input("Desea realizar otra transaccion: S/N > ").lower() # con lower() eliminamos el problema de mayuscula o minusculas
                 while SoN == "s":
                   opciones = (input("""\n    Digite el numero de alguna opciòn
1. Deposito
2. Retiro
3. Saldo
...
# condicionamos las opciones 
                   ...
                   if opciones == "2":
                      bil_2, aux_2, bil_5, aux_5, bil_10, aux_10, bil_20, aux_20, aux_retiro = 2000, 0, 5000, 0, 10000, 0, 20000, 0, 0
                      ...
                      if aux_retiro == 0 and retiro != 1000 and retiro != 3000:     # se realiza el retiro solo si esta en valores de 1000, no es 1000 ni 3000
                          if retiro <= saldo:
                              ...
                              if bil_retiro > 100000: print("No se puede dispensar ese  monto el cajero no tiene billetes de ",bil_20)
                              elif bil_retiro >= bil_10 and cant_10 == 0 and bil_retiro > 25000: print("No se puede dispensar ese  monto el cajero no tiene billetes de ",bil_10)  # verificamos tener billetes suficientes dependiendo del monto
                              else:     
                                  while bil_retiro >= bil_10 and cant_10 > 0:     
                                      bil_retiro -= bil_10
                                      aux_10 += 1
                                      cant_10 -= 1                                     
                                  if bil_retiro >= bil_5 and cant_5 == 0: print("No se puede dispensar ese  monto el cajero no tiene billetes de ",bil_5)
                                  else:    
                                      while bil_retiro >= bil_5 and bil_retiro != 8000 and bil_retiro != 6000 and cant_5 > 0:
                                          bil_retiro -= bil_5
                                          aux_5 += 1
                                          cant_5 -= 1
                                          # verificacion la cantidad e billetes
                                      if bil_retiro >= 2000 and cant_2 < 4 or bil_retiro > 10000: print("No se puede dispensar ese monto el cajero no tiene billetes de ",bil_2)
                                      else:    
                   ...
                   if opciones == "4":
                      print("\nCuentas favoritas: ")
                      print(" > ",separador.join(transferencia)," < ")   # mostramos la lista de las cuentas favoritas con separador
                      cliente_trans = input("A quien desea realizar la transferencia? : > ").upper()
                   ...
                   if opciones == "5":
                      ...
                      serv_pagar = str(input("Cual servicio desea pagar: > ").upper())
